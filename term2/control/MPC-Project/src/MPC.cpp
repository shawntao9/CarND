...
//set costs for variables, higher means get penalized more
double cost_cte = 2000;
double cost_epsi = 1000;
double cost_v = 1;

double cost_delta = 500;
double cost_a = 50;

double cost_delta_rate = 5000000;
double cost_a_rate = 10;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    //main cost: cte, heading error, velocity error
    for(int i = 0; i < N; i++){
    ...
    for(int i = 0; i < N - 2; i++){
      fg[0] += cost_delta_rate * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += cost_a_rate * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
    ...
    for (int t = 1; t < N; ++t) {
      ...
      AD<double> f0 = coeffs[0] + x0 * coeffs[1] + coeffs[2] * CppAD::pow(x0, 2) + coeffs[3] * CppAD::pow(x0, 3);
